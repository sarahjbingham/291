...
>>>>>>> 80d0af210ec5609e22ca88215ad6e3662a9b446a

Our group analyzed two separate models, full and reduced; however, the coefficients for **every** variable, regardless of model, was found to be significant ($p < .05$). After picking a reduced model that omitted weight and computer use frequency, we ran a nested F-test to determine whether the full model was necessary since all the coefficients had very small P valuges. Our significant F-test determined the full model was a stronger model and that the two extra variables should be retained ($F < .05$, $p < .05$).  
