...
Our project seeks to help understand the environmental conditions that effect the average individual's sleep per night during the Covid-19 pandemic. Our study examines how increased screen time, medication for depression, sleep medication, number of days alcohol is consumed within a week may effect the amount of time a person sleeps each night. Individuals stuck at home may be at a greater risk for depression, insomnia, substance use disorder, and of course are more likely to spend a greater amount of time on their computers. Our project uses data collected in 2018 from the National Health Interview Survey (IPUMS-NHIS) to see whether these variables have an effect on average sleep per night. Both a full and reduced model examined whether these variables had significant effects on the outcome variable of average sleep per night. Though each variable was, in fact, found be significant predictors of sleep length, we believe our large data set means these findings will have very little practical implications for the general public.  
...
```{r}
sum(hellostats_data$RACENEW == 10)
```

We began with a large national data set of 72,831 people. After filtering for non-applicable responses, omitting variables with more than two categories, and computer use frequency data, 11,672 individuals remained. Average amount of sleep per night is 7.08 with an average weight of 177.49. The data consisted of overwhelmingly White participants, with 84.21% of individuals self-identifying as White, and 9.07% as Black or African-American, 3.49% as Asian, 1.15% Alaskan Native/American Indian, 2.08% as multiple race, and 0% as other. It should therefore be noted that this data set may does not sufficiently represent and may not therefore be generalizeable to non-White-identifying people.  

Our group ran two models, a full and reduced model; however, the coefficients for *every* variable, regardless of model, was significant ($p < .05$). After picking a reduced model that omitted weight and computer use frequency, we ran a nested F-test to determine whether the full model was necessary. Our significant F-test determined the full model was a stronger model and that the two extra variables should be retained ($F < .05$, $p < .05$).  

Finally, we believe the extremely small p-values mean that though all our coefficients were technically significant predictors of the average amount of sleep per night, this is in part due to the extremely large data set ($N = 72,831$). Despite the significance of these predictors, we believe the actual practical application of these findings needs a more specialized study. For instance, the more frequently alcohol is consumed throughout the week is significantly positively correlated to an increase in average time slept per night, this study fails to account for the **quality** of said sleep. With such a large data set, it is likely that the significance of sleep time could so small as to be inconsequential; i.e., an 10 minutes of extra sleep per night may be statistically significant in our model, but it is inconsequential to the individual's overall rest.  

# Discussion  

The main goal of our analysis is to assess the relationship between sleep duration and computer use in relation to sleep and depression medication, alcohol, and weight. Based on the full model, we can infer the relative effect of each variable on the sleep duration. One additional sleep medication taken in one week decreases the hours slept by 0.03 hour, which is not a high drop in sleep duration.
